...
// REMOVIDA a importação da biblioteca 'iframe-resizer-react'
     ...
     <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black z-50 flex flex-col">
      ...
      <div className="flex-1 bg-black">
        {!showExercises ? (
            ...
            // Usando um iframe padrão que ocupa 100% da altura
            <iframe 
      ...
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex items-center justify-between bg-[#1E1E1E] rounded-lg p-1 border border-gray-700">
            ...
            <button onClick={toggleFullscreen} className="flex items-center space-x-2 bg-[#272525] hover:bg-gray-700 px-4 py-3 rounded-lg transition-colors ml-2" title="Expandir para tela cheia">
                <Maximize className="w-4 h-4" /><span className="hidden sm:block">Tela Cheia</span>
            </button>
        </motion.div>
        
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.4 }} className="bg-[#1E1E1E] rounded-xl overflow-hidden border border-gray-700">
            {!showExercises ? (
                <div className="relative"><div className="aspect-video bg-black"><iframe src={module.videoUrl} className="w-full h-full" frameBorder="0" allowFullScreen title={`Aula - ${module.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" /></div></div>
            ) : (
                <div className="p-6">
                    ...
                    <div className="bg-[#272525] rounded-lg border border-gray-600">
                        {/* Usando um iframe padrão com altura generosa */}
                        <iframe
                            src={`/exercises/${module.exerciseFile}?moduleId=${module.id}`}
                            className="w-full h-[80vh] rounded-lg" // Altura de 80% da tela
                            style={{ border: 'none' }}
                            title={`Exercícios - ${module.title}`}
                        />
                    </div>
                    <div className="mt-4 p-4 bg-blue-500/10 border border-blue-500 rounded-lg"><p className="text-blue-400 text-sm">💡 <strong>Dica:</strong> Complete os exercícios para reforçar seu aprendizado. As respostas são verificadas automaticamente no próprio exercício.</p></div>
                </div>
            )}
        </motion.div>
